...
export async function getSet (dataUrl) {
  const url = `${dataUrl}/lines?q=${parentfolder.value}&q_fields=parentfolder`
  const params = {
    method: 'GET',
    headers: {
      'Content-type': 'application/json'
...
// if file : if we just change name we do a simple patch else we do a post with more parameters in the dataform (create, id and attachmentPath)
